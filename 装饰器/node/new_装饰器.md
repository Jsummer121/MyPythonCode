...
其实上面可以看出，在进行运行装饰器的时候是按照从下往上走的，而执行函数的是是从上往下走的。形象点可以说是按照顺时针走，专业点就是如果找到装饰器，那就将这个装饰器压入栈，如果下面的还是装饰器就继续压入如果不是，就从栈中开始取装饰器执行代码，一个装饰器执行完成之后就继续压入一个新的栈，等到第一个栈空了以后就等待后面的程序运行该函数，如果运行了就继续从栈中取装饰器然后依次运行即可。
